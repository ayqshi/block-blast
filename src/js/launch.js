window._CCSettings = {
	...
	remoteBundles: [],
	subpackages: [],
	launchScene: "db://assets/Scene/blast.fire",
	orientation: "",
	jsList: []
};
(function() {
    ...
    //This is what handles resMap.js (otherwise, literally handles the whole game code) 
    //This is also the reason you are required to run bundle_resMap.js after each update/patch
    //If you wish to make resMap compatible with file pathing instead of the unreadable bs, below is indubitably relavent
	if (window.document) {
		...
		function loadArrayBuffer(url, options, onComplete) {
			let data = base64toArrayBuffer(window.resMap[url]);
			onComplete(null, data);
		}

		function loadText(url, options, onComplete) {
			options.responseType = "text";
			const data = window.resMap[url];
			onComplete(null, data);
		}

		cc.assetManager.downloader.register('.bin', loadArrayBuffer);
		...
		cc.assetManager.downloader.register('.plist', loadText);
